...
- **Leave and Company HQ are marked once for the whole period.** When an MR picks *On leave* or *Company HQ*, a From/To calendar appears. The backend writes one row per day in that range (`LEAVE` or `HQ`), so the board shows them out of the field every day without anyone re-marking — and the office isn't left calling someone who's on holiday or at head office. Working any of those days simply overwrites that day's row. Ranges are capped at 92 days.
